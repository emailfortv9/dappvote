...
contract("Election", function(accounts) {
  ...
  it("it initializes the candidates with the correct values", function() {
    return Election.deployed().then(function(instance) {
      ...
      assert.equal(candidate[1], "Edappadi Palaniswami", "contains the correct name");
      assert.equal(candidate[2], "AIADMK", "contains the correct party");
      assert.equal(candidate[3], 0, "contains the correct votes count");
      return electionInstance.candidates(2);
    }).then(function(candidate) {
      assert.equal(candidate[0], 2, "contains the correct id");
      assert.equal(candidate[1], "MK Stalin", "contains the correct name");
      assert.equal(candidate[2], "DMK", "contains the correct party");
      assert.equal(candidate[3], 0, "contains the correct votes count");
      return electionInstance.candidates(3);
    }).then(function(candidate) {
      assert.equal(candidate[0], 3, "contains the correct id");
      assert.equal(candidate[1], "Arvind Kejriwal", "contains the correct name");
      assert.equal(candidate[2], "Aam Aadmi Party", "contains the correct party");
      assert.equal(candidate[3], 0, "contains the correct votes count");
      return electionInstance.candidates(4);
    }).then(function(candidate) {
      assert.equal(candidate[0], 4, "contains the correct id");
      assert.equal(candidate[1], "Mamata Banerjee", "contains the correct name");
      assert.equal(candidate[2], "Trinamool Congress", "contains the correct party");
      assert.equal(candidate[3], 0, "contains the correct votes count");
